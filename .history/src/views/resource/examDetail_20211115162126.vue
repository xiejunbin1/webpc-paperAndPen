...
<section class="exam-detail">
    <div id="pdfDom" class="exam-detail__body">
        <div v-for="(section, si) in list" :key="si">
            ...
            <question-item :up="ei > 0" :down="ei < section.sectionExamList.length - 1" :is-send="$route.query.fromTask" @add="handleAdd(si, ei, exam)" @changeItem="changeItem" v-for="(exam, ei) in section.sectionExamList" :key="ei" @setPoint="setPoint($event, exam, si, ei)" @move="handleMove($event, section.sectionExamList, ei, si)" :item="exam.examQuestion" @correct="
