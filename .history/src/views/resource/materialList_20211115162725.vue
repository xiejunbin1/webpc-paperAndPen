...
<section class="material-list-wrap">
    ...
    <div class="" ref="body">
        <van-pull-refresh v-model="refLoading" @refresh="onRefresh">
            <div v-if="!listLoading && list.length == 0" class="null-tips-div" style="text-align: center; color: #999999; padding-top: 100px">
            ...
            <van-list v-model="listLoading" :finished="finished" :finished-text="list.length > 0 ? '没有更多了' : '当前没有素材，快去上传吧！'" @load="onLoad" :offset="80">
                <list-item ref="listItem" @clickTo="goto(item)" class="mgt10 common_list" style="background: #fff" @del="modifyTeachCourseRes(item, index)" @clickDel="clickDel(index)" v-for="(item, index) in list" :key="index" :itemTitle="item.coursewareName" :can-slide="true">
                    ...
                    <!-- <div slot="cover" class="cover" :style="{ background: item.imageUrl ? 'none' : '#f3d233' }">
              <img v-if="item.imageUrl" :src="item.imageUrl" alt="" /><i v-else :class="['iconGFY', handleIcon(item)]"></i>
            </div> -->
                    <div slot="cover" class="cover paper_logo lectureList-cover-icon">
                        <svg-icon :icon-class="handleIcon(item).class" />
                        <span class="lectureList-cover-icon-title">{{ handleIcon(item).title }}：</span>
                    </div>
                    <div slot="desc">
                        <!-- <div class="desc-top">
                ...
                <div><i class="iconGFY icon-feather"></i>{{ item.belongAccountName }}</div>
                        ...
                        <!-- <div class="desc-bottom" style="margin-top: 4px">
                <div style="width: 30%"><i class="iconGFY icon-download"></i>{{ item.downCount || 0 }}</div>
                <div style="width: 30%"><i class="iconGFY icon-points"></i>{{ item.useCount || 0 }}</div>
                <div style="width: 30%"><i class="iconGFY icon-star"></i>{{ item.collectCount || 0 }}</div>
              </div> -->
                    </div>
              ...
              <div @click="sendTask(item)">
                ...
                <span>发任务</span>
                    ...
                    <div slot="icon-go" class="dropdown-header-item-type-icon">
                        ...
                        <!-- PC端做推送，web端先隐藏 -->
                        <!-- <span class="CaruseSend_img" v-if="jugeType(item) == 'img'" @click.stop="CaruseSendImg(item)">推送</span> -->
                    </div>
                </list-item>
            </van-list>
        </van-pull-refresh>
    ...
    methods: {
        CaruseSendImg(item) {
            ...
            let data = {
                ...
                // resourceType: item.coursewareType,
                resourceType: 'R01',
        ...
        jugeType(item) {
            ...
            let t = '';
            let type = '';
            t = item.srcUrl.substring(item.srcUrl.lastIndexOf('.') + 1).toLowerCase();
        ...
        gotoUpload() {
            ...
            this.$router.push({
                ...
                query: {
                    ...
                    tchClassCourseInfo: this.$route.query.tchClassCourseInfo,
        ...
        async download(item) {
            try {
                MobclickAgent.onEvent('resourceMaterialDownload');
            } catch (e) {
                console.log(e);
            }
            let url = item.srcUrl;
        ...
        downLoadToOpen(item) {
            var _this = this;
            // 文件后缀
            var type = item.srcUrl.substring(item.srcUrl.lastIndexOf('.') + 1).toLowerCase();
            ...
            fileTransfer.onprogress = function (progressEvent) {
                if (progressEvent.lengthComputable) {
                    let downloadProgress = ((progressEvent.loaded / progressEvent.total).toFixed(2) * 100).toFixed(2);
                    ...
                    entry.file((data) => {
                        ...
                        cordova.plugins.fileOpener2.showOpenWithDialog(
                            ...
                            function onError(error) {
                                console.log('出错！请在' + cordova.file.externalDataDirectory + '目录下查看');
                            }
                        );
                    });
                },
        ...
        goto(item) {
            ...
            // if (this.handleIcon(item).title == '视频' || this.handleIcon(item).title == '图片' || this.handleIcon(item).title == '音频') {
            //   this.$toast('功能正在开发中，敬请期待...');
            //   return;
            ...
            if (this.handleIcon(item).title == '音频') this.$router.push({
                path: '/materialDetail'
            });
        ...
        getList() {
            ...
            console.log('this.$store.getters.getBaseData', this.$store.getters.getBaseData);
            let obj = {
                interUser: 'runLfb',
                ...
                coursewareClassify: 'C01|C03|C04',
                currentPage: page,
            };
            let params = {
                requestJson: JSON.stringify(obj),
        ...
        sendTask(obj) {
            ...
            try {
                MobclickAgent.onEvent('materialAddTask');
            } catch (e) {
                console.log(e);
            }
