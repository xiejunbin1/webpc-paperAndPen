<template>
  <section class="exam-detail">
    <div id="pdfDom" class="exam-detail__body">
      <div v-for="(section, si) in list" :key="si">
        <div class="exam-detail__body__question-num">
          <div style="font-weight: bold">
            {{ numToWord(si + 1) }}.{{ section.testPaperSectionInfo.sectionName }}<span>(共{{ section.testPaperSectionInfo.sectionScore }}分)</span>
        ...
        <question-item
          :up="ei > 0"
          :down="ei < section.sectionExamList.length - 1"
          :is-send="$route.query.fromTask"
          @add="handleAdd(si, ei, exam)"
          @changeItem="changeItem"
          v-for="(exam, ei) in section.sectionExamList"
          :key="ei"
          @setPoint="setPoint($event, exam, si, ei)"
          @move="handleMove($event, section.sectionExamList, ei, si)"
          :item="exam.examQuestion"
          @correct="
            correctInfo = exam.examQuestion;
            correctShow = true;
          "
          :index="si + '-' + ei"
          :showTooltip.sync="exam.examQuestion.showTooltip"
          :showDel.sync="exam.examQuestion.showDel"
        >
...
import { updateTestPaperSectonIndex, getTestPaperExamInfo, updateTestPaperSubExamScore, updateTestPaperExamScore, delTestPaperExamInfo } from '@/api/index';
import { numToWord } from '@/utils/filter';
import eventBus from '@/utils/eventBus';
// import downloadPDF from '@/utils/htmlToPdf.js';
// import mixin from '@/utils/mixin.js';
// import wordExport from '@/utils/export';
...
export default {
  ...
  data() {
    return {
      ...
      clickItem: '',
      testPaperInfo: '',
      testPaperSectionList: '',
      isfEducation: this.$route.query.isfEducation,
    };
  },
  methods: {
    downLoad() {
      $('#pdfDom').wordExport(this.$route.query.title); //参数是下载的word文件名
    },

    examDetailSend() {
      console.log(this.testPaperSectionList,"this.testPaperSectionList")
      let data = {
        exeamSendType: this.$store.getters.getPushstatus == '2' ? 'class' : 'layered',
        tchCourseId: this.$store.getters.getBaseData.tchCourseId,
        resourceName: this.$route.query.title,
        resourceId: this.$route.query.testPaperId,
        duration: this.$route.query.duration,
        testPaperSectionList: this.testPaperSectionList,
      };
    ...
    setScoreForView(scoreValue) {
      ...
      if (this.secArr.length) {
        ...
        if (this.childIndex === undefined) {
          ...
          //修改大题分数
          this.list[this.sectionIndex].sectionExamList[this.examIndex].sectionExamInfo.examScore = this.list[this.sectionIndex].sectionExamList[this.examIndex].examQuestion.groupExamList.reduce(
            (t, v) => {
    ...
    getDetail() {
      ...
      getTestPaperExamInfo(params).then((res) => {
        ...
        if (res.flag) {
          ...
          this.testPaperInfo = res.testPaperInfo;
          this.testPaperSectionList = res.testPaperSectionList;
  ...
  created() {
    console.log('试卷详情-----', window.location.href);
    this.getDetail();
  },
  mounted() {
