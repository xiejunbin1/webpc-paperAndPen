...
<section class="question-list">
    ...
    <div class="question-list__tab">
        <div v-if='!isQuestionType'>
            <div class="dropdown__title" @click="tab.questionType=!tab.questionType">
                <div class="ellipsis">{{ tab.questionTypeList.find(v => v.active) ? tab.questionTypeList.find(v => v.active).examTypeName:'题型' }}
                </div>
        ...
        <div>
            <div class="dropdown__title" @click="tab.difficult=!tab.difficult">
                <div class="ellipsis">{{tab.difficultList.find(v => v.active)?tab.difficultList.find(v => v.active).name:'难度'}}
                </div>
    ...
    <div class="question-list__body" ref="body">
        <van-pull-refresh v-model="refLoading" @refresh="onRefresh">
            ...
            <van-list v-model="listLoading" :finished="finished" :finished-text="list.length>0?'没有更多了':'当前没有试题～'" @load="onLoad" :offset='80'>
                <question-item ref="child" @add="handleAdd($event,item,index)" @correct="correctInfo=item;correctShow=true" :is-question="true" :is-send="false" v-for="(item,index) in list" :key="index" :item="item" :index="index" @changeItem="changeItem" :showTooltip.sync='item.showTooltip' :showDel.sync="item.showDel"></question-item>
            </van-list>
        </van-pull-refresh>
    ...
    <exam-bar ref="examBar" @setQuestionSelect="clear" :can-add-course="isRes" v-model="selectList" @clear="clear" :can-select="isRes?false:true" :qesTypeName='qesTypeName' :knowledgePoint='knowledgePoint' :isfEducation='isfEducation' :special='special'></exam-bar>
</section>
</template>
        ...
        handleAdd(isRemove, item, aa) {
            if (isRemove) {
                ...
                if (index > -1) {
                    ...
                    if (sectionIndex == -1) {
                        this.$toast('当前学科不含该题型，无法组卷！')
                        this.$nextTick(() => {
                            this.$refs.child[aa].childClick(0)
                        })
                        return
                    } else {
