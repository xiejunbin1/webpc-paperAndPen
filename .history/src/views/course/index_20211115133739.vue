<template>
  <section class="preview-wrap">
    ...
    <dropdown-header
      class="preview-wrap-content"
      ref="dropdown"
      v-show="courseList.length || firstFlag"
      :list="courseList"
      :course-name="courseName"
      :tch-course-id="tchCourseId"
      :refLoading.sync="dropdownRefLoading"
      :listLoading.sync="dropdownListLoading"
      :finished="dropdownFinish"
      @onLoad="dropdownOnLoad"
      @refresh="dropdownRefresh"
      @selectCourse="goDetail"
    >
    </dropdown-header>
  </section>
</template>
...
import { getClassTeachCourseInfo, getCourseTaskList, getClassStudent } from '@/api/index';
...
export default {
  ...
  data() {
    return {
      ...
      actions: [{ text: '上学期' }, { text: '下学期' }, { text: '全部' }],
      socket: '',
      showPage: false,
    };
  },
  ...
  methods: {
    ...
    async goDetail(tchCourseInfo, index) {
      ...
      const { tchCourseId, sysCourseId, relationCourseId, subjectType, classId, tchClassCourseInfo, classGrade, termType, courseName } = tchCourseInfo;
      this.$router.push({
        name: 'lectureList',
        query: {
          tchCourseId,
