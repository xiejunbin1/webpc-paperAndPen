...
<section class="preview-wrap">
    <iframe src="unix ../../assets/vue脚手架搭建.pdf" width=900 height=900></iframe>
    <!-- <object data="" type="application/pdf" width=800 height=800></object> -->
    ...
    methods: {
        ...
        checkUrlPermission(url, title) {
            ...
            pubApi.checkUrlPermission({requestJson: JSON.stringify(permissionParams)}).then((respone) => {
                ...
                !assUrl ? this.$toast('暂无资源') : this.$router.push({name: 'videoPage',query: {src: assUrl,title,},});
            }).catch(() => {
                this.$toast('资源错误');
            });
        },
