  ...
  <section class="in-class-wrap">
    <div class="in-class-wrap__body">
      ...
      <!-- <div v-if="courseList.length || firstFlag">
        <van-cell class="fs16" :title="`讲义(${handleCount('R12_C01')})`" is-link @click="goto('/lectureList', 'clickInClassLecture')" />
        <van-cell class="fs16" :title="`白板(${handleCount('R12_C02')})`" is-link @click="goto('/boardClassList', 'clickInClassBoard')" />
        <van-cell class="fs16" :title="`堂测统计(${handleCount('R12_C03')})`" is-link @click="goto('/classStatSelectList', 'clickInClassStat')" />
      </div> -->
  ...
  methods: {
    ...
    goto(name, event) {
      ...
      if (name === 'classStatSelectList') {
        ...
        // localStorage.setItem('taskTchCourseInfo', this.$route.query.tchCourseInfo);
      }
      // try {
      //   MobclickAgent.onEvent(event);
      // } catch (e) {
      //   console.log(e);
      // }
      // console.log('tchCourseInfo', tchCourseInfo);
      // const { tchCourseId, sysCourseId, relationCourseId, subjectType, classId, tchClassCourseInfo, classGrade, termType, courseName } = JSON.parse(this.$route.query.tchCourseInfo);
