...
export default {
    ...
    data() {
        return {
            ...
            firstFlag: true,
            resourceCount: [{
                    resourceType: 'R12_C01',
                    resourceCount: 0
                },
                {
                    resourceType: 'R12_C02',
                    resourceCount: 0
                },
                {
                    resourceType: 'R12_C03',
                    resourceCount: 0
                },
            ],
        };
    ...
    methods: {
        ...
        goto(name, event) {
            ...
            const {
                ...
                termType,
                courseName
            } = JSON.parse(localStorage.getItem('tchCourseInfo'));
        ...
        async changeCourse(type) {
            ...
            if (type) {
                if(this.index < this.courseList.length - 1) this.index++; this.sharedAssignment();
                //下一题
                if (this.index >= this.courseList.length - 1) {
                    // 当前课程已是列表的最后一个
                    !this.dropdownFinish ? (await this.dropdownOnLoad(),this.index++,this.sharedAssignment()) : this.$toast('没有下一课了');
                    // if (!this.dropdownFinish) {
                    //     //还能加载下一页
                    //     await this.dropdownOnLoad();
                    //     this.index++;
                    //     this.sharedAssignment()
                    // } else {
                    //     //最后一页
                    //     this.$toast('没有下一课了');
                    // }
                ...
                //上一题
                this.index <= 0 ? this.$toast('没有上一课了') : this.index--;this.sharedAssignment()
            }
        },
        ...
        async dropdownOnLoad() {
            this.dropdownPage++;
            if (this.dropdownPage > this.dropdownTotal && this.dropdownPage > 1) return;
