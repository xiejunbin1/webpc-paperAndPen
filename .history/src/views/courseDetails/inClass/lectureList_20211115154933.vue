...
<section class="lecture-list">
    <div class="common_body" ref="body">
        <van-pull-refresh v-model="refLoading" @refresh="onRefresh">
            ...
            <van-list v-model="listLoading" :finished="finished" :finished-text="list.length > 0 ? '没有更多了' : '当前没有讲义，快去选择课件吧！'" 
            @load="onLoad" :offset="80">
                <list-item ref="listItem" v-if="renderFlag" @clickTo="goto(item.ClassTeachingData)"
                class="mgt10 common_list" style="background: #fff" @del="handleDelete(item.ClassTeachingData, index)"
                @clickDel="clickDel(index)" v-for="(item, index) in list" 
    ...
    methods: {
        ...
        jugeType(item) {
            ...
            if (t === 'ppt' || t === 'pptx') {
                ...
                type = 'pdf';
            } else if (['jpg','png','jpeg'].indexOf(t) != -1) {
                type = 'img';
            } else if (t == 'mp4' || t == 'mp3') {
                type = 'video';
            }
