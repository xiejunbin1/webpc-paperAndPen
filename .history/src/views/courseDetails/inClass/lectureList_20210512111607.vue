<template>
  <section class="lecture-list">
    <div class="lecture-list__body common_body" ref="body">
      <van-pull-refresh v-model="refLoading" @refresh="onRefresh">
        ...
        <van-list v-model="listLoading" :finished="finished" :finished-text="list.length > 0 ? '没有更多了' : '当前没有讲义，快去选择课件吧！'" @load="onLoad" :offset="80">
          <list-item
            ref="listItem"
            v-if="renderFlag"
            @clickTo="goto(item.ClassTeachingData)"
            class="mgt10 common_list"
            style="background: #fff"
            @del="handleDelete(item.ClassTeachingData, index)"
            @clickDel="clickDel(index)"
            v-for="(item, index) in list"
            :key="item.ClassTeachingData.seqId + '' + item.ClassTeachingData.resourceId"
            :itemTitle="item.ClassTeachingData.name"
            @up="moveItem(item.ClassTeachingData, index, 0)"
            @down="moveItem(item.ClassTeachingData, index, 1)"
            :can-slide="true"
            :up="index > 0"
            :top="list.length > 1 && index > 0"
            :down="list.length - 1 > index"
            @top="topItem(item.ClassTeachingData, index)"
          >
            <span class="list-index" slot="list-index">{{ index + 1 }}、</span>
            <!-- <div slot="cover" class="cover" :style="{ background: handleBackground(item) }">
              <i class="iconGFY" :class="handleIcon(item.ClassTeachingData)"></i>          
            </div> -->
  ...
  methods: {
    ...
    goto(item) {
      if (item.resourceType === 'R01') {
        ...
        if (t == 'mp3') this.$router.push({ name: 'boardDetail' });
      } else {
        //试卷
        this.$router.push({
          name: `examDetail`,
