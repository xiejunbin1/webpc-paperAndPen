    ...
    <div class="class-stat-list__body" ref="body">
      <van-pull-refresh v-model="refLoading" @refresh="onRefresh">
        ...
        <van-list v-model="listLoading" :finished="finished" :finished-text="list.length > 0 ? '没有更多了' : '当前没有内容～'" @load="onLoad" :offset="80">
          ...
          <list-item
            ref="listItem"
            @clickTo="viewStat(item)"
            class="mgt10 common_list"
            style="background: #fff"
            @del="handleDelete(item, index)"
          ...
          >
            <!-- <div slot="cover" class="cover">
              <img src="../../../assets/img/course/course_paper.png" alt="" slot="listItem_img" class="classStatList_listItem_img" />
            </div>
            <div class="classStatList_listItem_desc" slot="desc">
              <div v-if="item.sendRangeName" class="desc-bottom">{{ item.sendRangeName }}</div>
              <div v-else class="desc-bottom">
                <div v-for="(c, ci) in item.tchClassTastInfo" :key="ci">{{ c.startDate }} -- {{ c.endDate }}</div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    goInClassDetail(item) {
      this.$router.push({ name: 'inClassDetail' });
