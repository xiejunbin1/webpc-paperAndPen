...
<div class="inClassDetail" :class="{ hideAll: hideSomeThing }">
    <!-- {{ $store.getters.getExamDetailData.taskType }}{{ showTestPaper }}{{ isFromClassStatList }}{{ item.taskName }} -->
    <!-- {{ hideSomeThing }}{{ this.$route.query.hasOwnProperty('socketTaskId') }}{{ this.$route.query }} -->
    <!-- {{ this.$store.getters.getExamDetailData.from }} -->
    <!-- {{ this.$route.query.hasOwnProperty('socketTaskId') }} -->
    <!-- 试卷、点击推送 -->
    <div v-if="hideSomeThing" class="daya">
        ...
        <div class="inClassDetail_content">
            <!-- <ul class="courseDetails_footer_menu courseDetails_footer_menu_statistic">
          <li @click="goBack">
...
export default {
    ...
    methods: {
        ...
        goBack() {
            ...
            if (this.$route.name == 'subjectAnalyse' || this.$route.name == 'subjectList' || this.$route.name == 'questionDetail') {
                this.$router.go(-1);
            } else {
                this.$router.push({
        ...
        goRouter(name) {
            if (name == 'examDetail') {
                if (this.item.resourceType === 'R03') {
                    ...
                    //单道试题
                    this.$router.push(
                        `/inClassDetail/questionDetail?tchCourseId=${this.$store.getters.getBaseData.tchCourseId}&taskId=${this.$store.getters.getExamDetailData.info.taskId}&title=${this.item.info.taskName}`
                    );
                } else {
                    //试卷
                    this.$router.push({
