...
<div class="courseDetails">
    ...
    <ul class="courseDetails_footer_menu xuanfu" id="moveDiv" @mousedown="down" @touchstart="down" @mousemove="move" @touchmove.prevent="move" @mouseup="end" @touchend="end">
        ...
        <li :class="{ select_li: $route.path.indexOf('inClass') > 0 }" v-show="showCourseDetailsFooter" @click="goRouter('inClass')">
            <svg-icon icon-class="icon_caourse_class" /><span>课中</span></li>
        <li @click="goRouter('preview')" :class="{ select_li: $route.name == 'preview' }" v-show="showCourseDetailsFooter">
            <svg-icon icon-class="icon_caourse_task"></svg-icon><span>任务</span>
        </li>
        ...
        <li @click="goBack">
            <svg-icon icon-class="icon_caourse_back"></svg-icon><span>返回</span>
        </li>
    </ul>
</div>
</template>
