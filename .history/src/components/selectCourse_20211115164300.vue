<template>
<van-popup v-model="show" :closeable="true" close-icon="close" :close-on-click-overlay="false" position="bottom" :style="{ height: '93%' }">
                ...
                <div v-if="item.classGrade">
                    ...
                    <van-radio-group class="course-group" v-model="item.radio">
                        <van-radio @click="handleChange(index)" v-for="(c,ci) in item.arr" :key="ci" :name="ci" class="mgr10 mgb15">
                            <i slot="icon" slot-scope="props" :class="['iconGFY','icon-radio-active',{'radio-normal':!props.checked}]"></i>
