...
<section class="question-item-wrap">
    ...
    <div class="question-item-wrap__btn-group van-hairline--top" :id="'question-item' + index">
        ...
        <span><i class="iconGFY icon-star"></i>{{ item.collectCount || 0 }}</span> -->
            ...
            <!-- <div v-if="isQuestion" class="btn-item" :class="{ active: item.collectId }" @click="handleCollect(item.collectId)">{{ item.collectId ? '取消' : '添加' }}收藏</div>
        <div v-if="!isSend && !isQuestion && !isLec" class="btn-item" @click="$emit('setPoint')">设置分数</div>
        <div v-if="!isSend && !isLec" class="btn-item" :class="{ active: item.isRemove }" @click="handleAdd">{{ !item.isRemove ? '添加' : '移除' }}试题</div> -->
        </div>
    </div>
