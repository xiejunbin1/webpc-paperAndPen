...
<van-popup :close-on-click-overlay="false" v-model="show" position="bottom" :style="{ height: (type==='myCourse' || type==='error'||type==='fEducation')?'65%':'93%' }">
    <div class="course-filter-wrap">
        <van-overlay class-name="mask" :show="resourceDropdown||gradeDropdown||termDropdown||versionDropdown||classDropdown" @click="gradeDropdown = false;termDropdown=false;versionDropdown=false;classDropdown=false;resourceDropdown=false" />
        <!-- 资源 -->
        ...
        <div v-else class="course-filter-wrap__dropdown van-hairline--bottom">
            <div v-show="this.type!='myCourse'" style="width:45%;margin:0 auto;" class="course-filter-wrap__dropdown van-hairline--bottom">
                <div>
                    ...
                    <div v-show="resourceDropdown&&resourceNameLength.length>1" class="dropdown-menu">
                        <div class="dropdown-menu-item" :class="{active: resourceIndex== index}" v-for="(item,index) in resourceName" :key="index" @click="changeResource(index)" v-show="item.active">{{item.name}}
            ...
            <div v-if="resourceIndex==0">
                ...
                <div v-show="termDropdown" class="dropdown-menu">
                    <div class="dropdown-menu-item" @click="changeTermType(index)" :class="{active: termIndex== index}" v-for="(item,index) in termTypeList2" :key="index">{{item|getTermName}}
                        <van-icon v-show="termIndex== index " class="check blue" name="success" />
                    </div>
                </div>
            </div>
            ...
            <div v-if="resourceIndex==1||resourceIndex==2">
                ...
                <div v-if="classGradeList[gradeIndex].classGrade=='G09&1'">
                    ...
                    <div v-show="termDropdown" class="dropdown-menu">
                        <div class="dropdown-menu-item" @click="changeTermType(index)" :class="{active: periodIndex== index}" v-for="(item,index) in termTypeList2" :key="index">{{item|getPeriodName}}
                ...
                <div v-else>
                    ...
                    <div v-show="termDropdown" class="dropdown-menu">
                        <div class="dropdown-menu-item" @click="changeTermType(index)" :class="{active: periodIndex== index}" v-for="(item,index) in periodTypeList" :key="index">{{item|getPeriodName}}
                            <van-icon v-show="periodIndex== index " class="check blue" name="success" />
                        </div>
                    </div>
                </div>
            ...
            <div v-if="type!=='myCourse'||type!=='error'" class="course-filter-wrap__body-right">
                <div class="" v-for="(item,index) in courseList" :key="index">
                    <div v-if="item.childNodeList && item.childNodeList.length>0">
                        ...
                        <div :class="['course-sec',{active:currentSysCourseId == c.courseId}]" v-show="item.fold" @click="selectSysCourse(c.courseId,c.nodeName)" v-for="(c,ci) in item.childNodeList" :key="ci">
                            {{c.nodeName}}
    ...
    data() {
        return {
            ...
            resourceName: [{
                ...
                {
                    'name': '全朗智教',
                    'active': localStorage.getItem('ifQuanlangAccount') == 1 ? true : false
                },
                {
                    'name': '君韬教育',
                    'active': localStorage.getItem('ifJuntaoAccount') == 1 ? true : false
                }
            ],
