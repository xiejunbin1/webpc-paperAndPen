...
<div class="score-table">
    ...
    <el-table :data="listArr" width="100%" class="table-class" height="76vh" :border="false" stripe :header-cell-style="tableHeaderColor" v-show="classView">
        <!-- <el-table-column align="center" fixed min-width="180" prop="name" label="排名">
            <template >
                <span>11</span>
            </template>
        </el-table-column> -->
        ...
        <el-table-column align="center" min-width="160" prop="score" label="平均分">
            <template slot-scope="scope">
                <div>{{ scope.row.score | rounding }}</div>
            </template>
        </el-table-column>
        <el-table-column  align="center" min-width="120" v-for="(item, index) in examAnswerList" :key="index" :label="item.splitIndex + ''">
            <template slot-scope="scope">
                <div>{{ scope.row.splitAnswerList[index].score || 0  }}</div>
            </template>
        </el-table-column>
    ...
    <el-table type="index" width="100%" height="76vh" sort-orders="['ascending', 'descending']" @sort-change="sortChange" class="table-group"  :data="dataArr" stripe :header-cell-style="tableHeaderColor" v-show="!classView">
        ...
        <el-table-column label="学生姓名" align="center" prop="stu" width="170" v-if="groupArr.length > 0">
            <template slot-scope="scope">
                <div class="stu-class" style="display: flex;justify-content: center;" v-for="r in scope.row.stu">{{ r.name }} <div style="color: #fdb63f">({{ r.studentRewardScore }})</div> </div>
            </template>
        </el-table-column>
        ...
        <el-table-column align="center" min-width="120" v-for="(item, index) in examAnswerList1" :key="index" :label="item.splitIndex+''">
            <template slot-scope="scope">
                <div class="stu-class" v-for="(va, i) in scope.row.stu" :key="i">{{ va.splitAnswerList[index].score }}</div>
            </template>
        </el-table-column>
