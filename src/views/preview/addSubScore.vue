        ...
        <van-cell class="mgt10" v-for="(item, index) in scoreSpan" :key="index">
            <div slot="title">
                <div class="cell-header">
                    <van-checkbox :disabled="item.stu.length == 0" @click="handleChange($event, item)" class="cell-header__checkbox" :name="index + 1" v-model="item.check">
                        <i slot="icon" slot-scope="props" :class="['iconGFY', 'icon-check', { normal: !props.checked }]"></i>
                        <div class="fs16">
                            {{ item.name }}<span class="fs14">(已选{{ totalStu(item) }}人)</span>
                        </div>
                    </van-checkbox>
                ...
                <div class="cell-content" v-show="item.fold">
                    <div @click="handleItemClick(stu, item, index)" v-for="(stu, s) in item.stu" :key="s" :class="['cell-content-item', { active: stu.active }]">{{ stu.name }}</div>
                </div>
            </div>
        </van-cell>
    </div>
    <div class="add-sub-score__footer">
        <van-button type="info" class="btn" id="add" @click="
          scoreShow = true;
          leftValue = '30%';
        ">加分</van-button>
        <van-button type="info" class="btn" id="sub" @click="
          scoreShow = true;
          leftValue = '75%';
        ">减分</van-button>
    </div>
    ...
    methods: {
        saveRewardScore(score) {
            ...
            let obj = {
                ...
                termType: this.$store.getters.getUserInfo.termType,
                classId: this.$store.getters.getUserInfo.info.classId,
                groupIdList: '',
                accountNoList: str,
            };
            let params = {
        ...
        handleList() {
            ...
            if (this.$route.params.info.testPaperScore >= 20) {
                //总分不小于20
                this.scoreSpan = [{
                        name: `满分${this.$route.params.info.testPaperScore}`,
                        min: 1,
                        max: 2,
                        stu: []
                    },
                    ...
                    {
                        name: `${calculate.mul(this.$route.params.info.testPaperScore, 0.6, 0)}分以下`,
                        min: 0,
                        max: 0.6,
                        stu: []
                    },
